...
def server_GUI():
    ...
    login_btn = Button(front_page_server, text="Log in", command=lambda: [server_log_in(username.get(), password.get(), front_page_server)], font=("Arial", 18, "bold"))
    login_btn.place(x=350, y=375)

    register_btn = Button(front_page_server, text="Register", command=lambda: [server_register_page(front_page_server)], font=("Arial", 18, "bold"))
...
def server_register_page(front_page_server):
    front_page_server.withdraw()
    register_server = Toplevel()
    register_server.title("TeachaTool - Teacher")
    register_server.geometry("1280x720")
    register_server.iconbitmap("Logo.ico")
    register_server.resizable()

    welcome_lbl = Label(register_server, text="Welcome to TeachaTool", font=("Arial", 54, "bold"))
    welcome_lbl.place(relx=0.5, y=50, anchor=CENTER)

    register_lbl = Label(register_server, text="Register:", font=("Arial", 32, "underline"))
    register_lbl.place(x=350, y=150)

    username_lbl = Label(register_server, text="Username:", font=("Arial", 20, "underline"))
    username_lbl.place(x=350, y=250)

    new_username = StringVar()

    username_entry = Entry(register_server, textvariable=new_username, font=("Arial", 14))
    username_entry.place(x=500, y=255, height=30, width=200)

    password_lbl = Label(register_server, text="Password:", font=("Arial", 20, "underline"))
    password_lbl.place(x=355, y=300)

    password1 = StringVar()

    password_entry = Entry(register_server, textvariable=password1, show='*', font=("Arial", 16))
    password_entry.place(x=500, y=305, height=30, width=200)

    re_enter_password_lbl = Label(register_server, text="Re enter your password:", font=("Arial", 20, "underline"))
    re_enter_password_lbl.place(x=185, y=350)

    password2 = StringVar()

    re_enter_password_entry = Entry(register_server, textvariable=password2, show='*', font=("Arial", 16))
    re_enter_password_entry.place(x=500, y=355, height=30, width=200)

    register_btn = Button(register_server, text="Register", command=lambda: [server_register(new_username.get(), password1.get(), password2.get(), register_server, front_page_server)], font=("Arial", 18, "bold"))
    register_btn.place(x=365, y=425)

    login_btn = Button(register_server, text="Log in", command=lambda: [register_server.withdraw(), front_page_server.deiconify()], font=("Arial", 18, "bold"))
    login_btn.place(x=1000, y=425)

    register_lbl = Label(register_server, text="Already have an account?\nlog in here", font=("Arial", 10, "underline"))
    register_lbl.place(x=975, y=475)

    mainloop()


def server_register(username, password1, password2, register_server, front_page_server):
    ...
    if num == 0:
        ...
        register_server.withdraw()
        front_page_server.deiconify()


def server_log_in(username, password, front_page_server):
...
def start_main_page(front_page_server, username):
    ...
    teacher_message = Entry(main_server, textvariable=message, font=("Arial", 10))
    teacher_message.place(x=805, y=600, height=25, width=420)

    chat = Text(main_server)
    chat.place(x=775, y=20, height=575, width=450)
    chat.config(state=DISABLED)

    chat_scrollbar = Scrollbar(main_server)
    chat_scrollbar.place(x=1225, y=20, height=575)
    chat_scrollbar.config(command=chat.yview)

    student_list = Listbox(main_server)
    student_list.place(x=20, y=50, height=500, width=250)

    chat_combobox = ttk.Combobox(main_server, value="Everyone")
    chat_combobox.place(x=775, y=630)
    chat_combobox.current(0)

    screen_share_combobox = ttk.Combobox(main_server)
    screen_share_combobox.place(x=475, y=50)

    student_list_lbl = Label(main_server, text="Currently online:", font=("Arial", 14, "underline"))
    ...
    send_message_btn = Button(main_server, image=photo, command=lambda: [Server.Server.write(server,message.get(), chat, student_list, chat_combobox, screen_share_combobox), teacher_message.delete(0, 'end')], font=("Arial", 14, "bold"))
    send_message_btn.place(x=775, y=600, height=25, width=25)

    screen_share_btn = Button(main_server, text="Watch the screen of", command=lambda: [Server.Server.ask_for_stream(server, check_screen_share_receiver(screen_share_combobox))], font=("Arial", 12, "bold"))
    screen_share_btn.place(x=300, y=50)

    stop_screen_share_lbl = Label(main_server, text="To close a screen,\npress the window and then press ESC", justify=LEFT, font=("Arial", 12,))
    stop_screen_share_lbl.place(x=300, y=90)
    #stop_screen_share_btn = Button(main_server, text="Stop all screen shares", command=lambda: [Server.Server.stop_all_streams(server)], font=("Arial", 11, "bold"))
