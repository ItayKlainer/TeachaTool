...
class Server:
        ...
        if server_message.strip():
            ...
            if receiver == "Everyone":
                ...
                try:
                    ...
                    Server_GUI.disable_chat(chat)
                    server_message = "0" + self.server_name + "(Private): " + server_message
    ...
    def stop_all_streams(self):
        for i in range(len(self.name_socket_address_list)):
            self.name_socket_address_list[i][1].send("3".encode())
        self.stream_server.stop_server()
'''



    def check_connection(self, student_list, chat_combobox, screen_share_combobox):
        while True:
            for i in range(len(self.name_socket_address_list)):
                try:
...
def server_start(server, chat, student_list, chat_combobox, screen_share_combobox):
    ...
    listening.start()
    #checking_connection = threading.Thread(target=server.check_connection, args=(student_list, chat_combobox, screen_share_combobox,))
